...
def tab1_results(
    ...
    fig_heatmap = go.Figure(data=go.Heatmap(
        ...
        zmin=0,
        zmax=25,
        colorbar=dict(title='r_diff [kV·m/A]')
    ))

    fig_heatmap.update_layout(
        title=f'Differential Resistance r_diff(j, β)<br>Δ={Delta_meV:.0f} meV, τ={tau*1e12:.0f} ps, a_M={a_M*1e9:.1f} nm, F_c={F_c/1e3:.2f} kV/m',
        xaxis_title='j [A/m]',
        yaxis_title='β (filling fraction)',
    ...
    # =========================================================================
    # LINETRACES: Horizontal cuts (r_diff vs j at fixed β)
    # =========================================================================
    fig_linetraces_beta = go.Figure()

    # Select specific β values for linetraces
    ...
    fig_linetraces_beta.update_layout(
        title='Linetraces: r_diff(j) at Fixed Filling β',
        xaxis_title='j [A/m]',
        yaxis_title='r_diff [kV·m/A]',
        xaxis=dict(range=[0, j_limit]),
    ...
    # =========================================================================
    # LINETRACES: Vertical cuts (r_diff vs β at fixed j)
    # =========================================================================
    ...
    fig_linetraces_j.update_layout(
        title='Linetraces: r_diff(β) at Fixed Current j',
        xaxis_title='β (filling fraction)',
        yaxis_title='r_diff [kV·m/A]',
        yaxis=dict(range=[0, 30]),
        height=400,
        template='plotly_white',
    ...
    fig_combined_linetraces = _make_subplots(
        rows=1, cols=2,
        subplot_titles=('r_diff(j) at fixed β', 'r_diff(β) at fixed j'),
        horizontal_spacing=0.12
    )

    # Add β linetraces to left panel
    for _idx, beta_idx in enumerate(beta_indices):
        if beta_idx < len(beta_values):
            _beta = beta_values[beta_idx]
            _rdiff = heatmap_kV[beta_idx, :]
    ...
    fig_combined_linetraces.update_xaxes(title_text='β', row=1, col=2)
    fig_combined_linetraces.update_yaxes(title_text='r_diff [kV·m/A]', range=[0, 25], row=1, col=1)
    fig_combined_linetraces.update_yaxes(title_text='r_diff [kV·m/A]', range=[0, 25], row=1, col=2)
...
def tab2_algorithm(
    ...
    fig_dispersion_3d = go.Figure(data=[go.Surface(
        ...
        colorscale='Viridis',
        colorbar=dict(title='ε [meV]')
    )])
    fig_dispersion_3d.update_layout(
        title='C3-Symmetric Moire Band Dispersion ε(k)',
        scene=dict(
            xaxis_title='k_x',
            yaxis_title='k_y',
            zaxis_title='ε [meV]',
    ...
    _hs_points = {'Γ': (0, 0), 'M': (np.pi, 0), 'M\'': (0, np.pi)}
    ...
    fig_vel_components = make_subplots(rows=1, cols=2,
        subplot_titles=['v_x(k) = ∂ε/∂k_x', 'v_y(k) = ∂ε/∂k_y'])
    ...
    for _beta, _color in zip(_betas, _colors):
        ...
        fig_fermi_filling.add_trace(go.Contour(
            ...
            showscale=False, name=f'β={_beta}'
        ))
    fig_fermi_filling.update_layout(
        title='Fermi Contours at Different Fillings β',
        xaxis_title='k_x', yaxis_title='k_y',
    ...
    for _beta, _color in zip(_betas, _colors):
        ...
        fig_dos.add_hline(y=_eps_F, line_dash='dash', line_color=_color,
                         annotation_text=f'β={_beta}', annotation_position='right')
    fig_dos.update_layout(
        title='Density of States',
        xaxis_title='DOS [a.u.]', yaxis_title='ε [meV]',
        height=400, template='plotly_white'
    )
    ...
    fig_trajectory.add_trace(go.Contour(
        ...
        colorbar=dict(title='ε [meV]', x=1.15),
    ...
    fig_trajectory.add_trace(go.Scatter(
        ...
        marker=dict(size=8, color=_weights_traj, colorscale='Reds',
                   showscale=True, colorbar=dict(title='e^(-t/τ)', x=1.3)),
    ...
    fig_trajectory.update_layout(
        title='Trajectory Averaging: Trace Backward Along k(t) = k₀ - eFt/ℏ',
        xaxis_title='k_x', yaxis_title='k_y',
        height=450, template='plotly_white'
    )
    ...
    fig_weights.update_xaxes(title_text='t / τ')
    ...
    fig_field_direction.update_layout(
        title='Current Magnitude vs Field Direction (β=0.5)',
        polar=dict(
    ...
    fig_integrand = make_subplots(rows=1, cols=3,
        subplot_titles=['Occupation g(k)', 'Velocity v_x(k)', 'Integrand v_x·g'])
    ...
    - **Bandwidth**: $W = \frac{3\Delta}{2}$ (from minimum at Γ to maximum at K-points)
    ...
    - **Zero velocity**: At band extrema (Γ and K points)
    ...
    As β increases:
    - **β ≈ 0**: Small pocket around Γ (electron-like)
    - **β ≈ 0.5**: Large, trigonally-warped contour
    - **β → 1**: Small pockets around K-points (hole-like)
    ...
    **For Bloch oscillations**, trajectory averaging is essential because:
    ...
    3. **Integrand v_x·g**: Only occupied states contribute; note the partial cancellation!
    ...
    ```
    1. INPUT: β (filling), F_values (field sweep), physical parameters
    2. FOR each F in F_values:
       a. Compute ω_B = eF·a_M/ℏ
       b. FOR each k in BZ:
      i.  Trace trajectory backward: k(t) = k - ω_B·t·x̂
      ii. Compute g(k) = ∫ f₀(ε(k(t))) e^(-t/τ) dt/τ
       c. Integrate: j = ∫ v_x(k)·g(k) d²k
    3. OUTPUT: j(F) curve
...
def tab3_bz_evolution(go, mo, moire_dispersion, moire_velocity_x, np):
    ...
    fig_traj.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            colorscale='Reds',
            showscale=True,
            colorbar=dict(title='exp(-t/τ)', x=1.15)
        ),
        name='Trajectory'
    ))
...
def tab4_c3_symmetry(
    ...
    fig_compare.add_trace(go.Contour(
        ...
        y=_ky,
        colorscale='Viridis',
        colorbar=dict(title='ε [meV]', x=0.45),
        contours=dict(showlabels=True, labelfont=dict(size=10))
    ), row=1, col=1)
    ...
    fig_compare.add_trace(go.Contour(
        ...
        y=_ky,
        colorscale='Viridis',
        colorbar=dict(title='ε [meV]', x=1.02),
        contours=dict(showlabels=True, labelfont=dict(size=10))
    ), row=1, col=2)
    ...
    **Separable (Square):**
    ...
    The ratio $j_{2D}/j_{1D}$ depends on filling β and is typically 0.6-0.9.
        """)
    ])
    return (tab4_content,)
...
def tab5_parameters(
    ...
    |-----------|--------|-------|
    | Bandwidth | Δ | {Delta_meV:.0f} meV |
    | Scattering time | τ | {tau*1e12:.1f} ps |
    ...
    **1. Critical Field (independent of Δ!):**
    $$
    ...
    This depends only on geometry ($a_M$) and scattering ($\tau$), NOT on bandwidth!
        ...
        fig_Fc_tau,
        mo.md(r"""
    **2. Maximum Velocity (proportional to Δ):**
    $$
    ...
    **3. Saturation Current:**
    ...
    - **Δ controls current magnitude:** Bandwidth sets the velocity scale, but doesn't change when NDR occurs.
...
def tab6_explorer(
    ...
    fig_jf_selected.add_trace(go.Scatter(
        ...
        mode='lines',
        line=dict(width=3, color='blue'),
        name=f'j(F) at β={beta_selected:.2f}'
    ))
    ...
    fig_jf_selected.update_layout(
        title=f'Current-Field Curve at β = {beta_selected:.2f}',
        xaxis_title='F / F_c',
        yaxis_title='j [A/m]',
        height=400,
        template='plotly_white'
    ...
    fig_rdiff.update_layout(
        title=f'Differential Resistance at β = {beta_selected:.2f}',
        xaxis_title='j [A/m]',
        yaxis_title='r_diff [kV·m/A]',
        height=400,
        template='plotly_white'
    ...
    **Selected:** β = {beta_selected:.2f}
    ...
    The 2D numerical model shows similar shape but with reduced magnitude due to phase-space effects.
        ...
        fig_compare_1d,
        mo.md(rf"""
    ### Summary for β = {beta_selected:.2f}
...
def tab7_physics_insight(go, mo, moire_dispersion, moire_velocity_x, np):
    ...
    # At finite field, current is j ∝ ∫ v(k) f(k-eEt/ℏ) dk
    # For steady state with scattering, this becomes the Esaki-Tsu result

    # Simulate j_max vs β for 1D
    _j_max_1d = []
    for _beta in np.linspace(0.01, 0.99, 50):
        # Approximate: j_max ∝ β(1-β) for symmetric band
    ...
    # Angular average of cos(θ) over Fermi circle = 0 at equilibrium
    # Under field, the shift gives j ∝ β but reduced by ~1/2 compared to 1D
    ...
    fig_j_vs_beta.add_annotation(
        ...
        text=f"Optimal β ≈ {_beta_range[_opt_idx]:.2f}",
    ...
    fig_velocity_field.add_trace(go.Scatter(
        ...
        line=dict(color='blue', width=2), name='vₓ > 0 (+ current)'
    ))
    fig_velocity_field.add_trace(go.Scatter(
        x=[None], y=[None], mode='lines',
        line=dict(color='red', width=2), name='vₓ < 0 (− current)'
    ))

    fig_velocity_field.update_layout(
        title='C3 Velocity Field: Arrows Show v(k), Color = Sign of vₓ',
        xaxis_title='kₓ', yaxis_title='kᵧ',
    ...
    # For 1D comparison: all velocities at 0 or π
    _angle_hist_1d = np.zeros_like(_angle_hist)
    _angle_hist_1d[0] = 50  # All at θ=0 or π
    _angle_hist_1d[-2] = 50
    ...
    fig_angle_dist.update_layout(
        ...
        annotations=[
            dict(x=0.5, y=-0.15, xref='paper', yref='paper',
                 text='θ=0° is along E-field → only cos(θ) component contributes to current',
    ...
    # 1D: delta functions at 0 and π
    # 2D isotropic: cos(θ) distribution
    # 2D C3: cos(θ) with trigonal modulation
    ...
    def _compute_driven_distribution(_kx, _ky, _E, _E_F, _delta_k, _T_eff=0.005):
        ...
        Field shifts distribution: f(k) ≈ f₀(k - δk) with smearing.
        """
        ...
        # Smeared Fermi function (finite τ broadening)
        _f = 1 / (1 + np.exp((_E_shifted - _E_F) / (_T_eff + 0.001)))
        return _f

    # Different field strengths (δk = F*τ*e/ℏ ∝ F/F_c)
    ...
    fig_dist_3d.update_layout(
        title='Distribution Function f(kₓ, kᵧ) Under Driving Field',
        xaxis_title='kₓ', yaxis_title='kᵧ',
        xaxis=dict(tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'], scaleanchor='y'),
        yaxis=dict(tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π']),
        height=450,
        template='plotly_white',
    ...
    fig_jx_3d = go.Figure(
        data=[go.Contour(x=_kx_c3, y=_ky_c3, z=_jx_init,
                        colorscale='RdBu_r', zmid=0,
                        colorbar=dict(title='jₓ(k) [meV]', len=0.6),
                        contours=dict(showlines=True))],
        frames=_frames_jx_2d
    )

    fig_jx_3d.update_layout(
        title='Current Contribution jₓ(k) = vₓ(k)·f(k)',
        xaxis_title='kₓ', yaxis_title='kᵧ',
        xaxis=dict(tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'], scaleanchor='y'),
        yaxis=dict(tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π']),
        height=450,
    ...
    for _i in range(1, 5):
        fig_dist_heatmaps.update_xaxes(title_text='kₓ', row=1, col=_i,
                                       tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'])
        fig_dist_heatmaps.update_yaxes(title_text='kᵧ' if _i == 1 else '', row=1, col=_i,
                                       tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'],
    ...
    for _i_col, _dk in enumerate(_F_heatmap_vals):
        ...
        fig_jx_heatmaps.add_trace(
            go.Heatmap(x=_kx_c3, y=_ky_c3, z=_jx_contrib,
                      ...
                      colorbar=dict(title='jₓ [meV]', x=1.02) if _i_col == 3 else None),
            row=1, col=_i_col+1
        )

    fig_jx_heatmaps.update_layout(
        title='Current Integrand jₓ(k) = vₓ·f at Different Field Strengths (Blue=+, Red=−)',
        height=350,
        template='plotly_white'
    )
    for _i in range(1, 5):
        fig_jx_heatmaps.update_xaxes(title_text='kₓ', row=1, col=_i,
                                     tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'])
        fig_jx_heatmaps.update_yaxes(title_text='kᵧ' if _i == 1 else '', row=1, col=_i,
                                     tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'])

    # --- Figure 5: Filling dependence - distributions at different β ---

    _beta_viz = [0.2, 0.4, 0.6, 0.8]
    _E_F_from_beta = [0.02, 0.04, 0.06, 0.075]  # Approximate E_F for each β

    fig_filling_dist = _make_subplots_c3(
        rows=2, cols=4,
        subplot_titles=[f'β={b}, F=0' for b in _beta_viz] + [f'β={b}, F=Fc' for b in _beta_viz],
    ...
    _delta_k_range = _F_range_c3 * np.pi  # δk = F/Fc * π
    ...
    fig_jF_filling.update_layout(
        ...
        xaxis_title='F / Fc',
        yaxis_title='j / j_max (normalized per β)',
        height=400,
        template='plotly_white',
        legend=dict(x=1.02, y=1)
    )

    # --- Figure 7: 2D Phase diagram j(F, β) heatmap ---

    _n_F_pd = 40
    _n_beta_pd = 30
    _F_pd = np.linspace(0.1, 2.5, _n_F_pd)
    _beta_pd = np.linspace(0.1, 0.9, _n_beta_pd)
    _E_F_pd = np.linspace(0.01, 0.075, _n_beta_pd)  # E_F corresponding to β
    ...
    fig_phase_diagram_c3.update_layout(
        title='Phase Diagram: Current j(F/Fc, β) in 2D C3 System',
        xaxis_title='F / Fc',
        yaxis_title='β (filling)',
        height=500,
        template='plotly_white'
    )
    ...
    fig_current_vectors.add_trace(go.Scatter(x=[None], y=[None], mode='lines',
                                             line=dict(color='red', width=2), name='jₓ > 0'))
    fig_current_vectors.add_trace(go.Scatter(x=[None], y=[None], mode='lines',
                                             line=dict(color='blue', width=2), name='jₓ < 0'))

    fig_current_vectors.update_layout(
        title='Current Density Vector Field j(k) at F=Fc',
        xaxis_title='kₓ', yaxis_title='kᵧ',
        xaxis=dict(scaleanchor='y', tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π']),
        yaxis=dict(tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π']),
        height=500,
        template='plotly_white',
        legend=dict(x=1.02, y=1)
    ...
    for _i in range(1, 3):
        fig_eq_vs_driven.update_xaxes(title_text='kₓ', row=1, col=_i,
                                      tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'])
        fig_eq_vs_driven.update_yaxes(title_text='kᵧ' if _i == 1 else '', row=1, col=_i,
                                      tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π'],
                                      scaleanchor=f'x{_i}' if _i > 1 else 'x')

    # --- Figure 10: Animated trajectory visualization in 2D BZ ---

    # Show how Fermi surface carriers move under field
    _n_traj_pts = 50
    _theta_traj = np.linspace(0, 2*np.pi, _n_traj_pts, endpoint=False)

    # Sample points on Fermi surface
    _r_fermi = 0.8  # Approximate radius of Fermi surface in k-space
    _kx_start = _r_fermi * np.cos(_theta_traj)
    _ky_start = _r_fermi * np.sin(_theta_traj)

    _frames_traj_2d = []
    _t_frames = np.linspace(0, 2, 20)  # Time evolution

    for _t_val in _t_frames:
        # Under field, k shifts: kx(t) = kx(0) + F*t (in normalized units)
        _kx_t = _kx_start + _t_val * np.pi * 0.5  # Slower for visualization
        _kx_t_wrapped = ((_kx_t + np.pi) % (2*np.pi)) - np.pi

        _frame_data = [
            # Energy contours (background)
            go.Contour(x=_kx_c3, y=_ky_c3, z=_E_c3 * 1000,
                      colorscale='Greys', opacity=0.3, showscale=False,
            ...
            # Carrier positions
            go.Scatter(x=_kx_t_wrapped, y=_ky_start,
                      mode='markers',
                      marker=dict(size=8, color=np.cos(_theta_traj),
                                 colorscale='RdBu_r', cmin=-1, cmax=1),
                      name='Carriers')
        ]
        _frames_traj_2d.append(go.Frame(data=_frame_data, name=f'{_t_val:.2f}'))

    fig_trajectory_2d = go.Figure(
        data=[
            go.Contour(x=_kx_c3, y=_ky_c3, z=_E_c3 * 1000,
                      colorscale='Greys', opacity=0.3, showscale=False,
                      contours=dict(coloring='lines')),
            go.Contour(x=_kx_c3, y=_ky_c3, z=_E_c3,
                      contours=dict(start=_E_F_c3, end=_E_F_c3, size=0.001, coloring='lines'),
                      line=dict(color='green', width=2), showscale=False, name='Fermi surface'),
            go.Scatter(x=_kx_start, y=_ky_start, mode='markers',
                      marker=dict(size=8, color=np.cos(_theta_traj),
                                 colorscale='RdBu_r', cmin=-1, cmax=1),
                      name='Carriers (color = vₓ)')
        ],
        frames=_frames_traj_2d
    )

    fig_trajectory_2d.update_layout(
        title='Carrier Trajectories in 2D BZ Under Field (Click Play)',
        xaxis_title='kₓ', yaxis_title='kᵧ',
        xaxis=dict(scaleanchor='y', range=[-np.pi, np.pi],
                  tickvals=[-np.pi, 0, np.pi], ticktext=['-π', '0', 'π']),
        ...
        template='plotly_white',
        updatemenus=[dict(
            type='buttons', showactive=True, y=-0.1, x=0.5, xanchor='center',
            buttons=[
    ...
    **For a C3-symmetric band (like moiré graphene):**
    ...
    **Combined effect for C3 2D system:**
    ...
    This explains why observing Bloch oscillations in 2D moiré systems requires:
...
@app.cell
def load_bandwidth_data(np, os):
    """Load heatmap data for multiple bandwidths."""
    bandwidth_data = {}

    for _bw_Delta in [50, 60, 70]:
        _filename = f'heatmap_2d_Delta{_bw_Delta}meV'
        _bw_file = None

        if os.path.exists(f'{_filename}.npz'):
            _bw_file = np.load(f'{_filename}.npz')
        else:
            _bw_file = load_embedded(_filename)

        if _bw_file is not None:
            bandwidth_data[_bw_Delta] = {
                'beta_values': _bw_file['beta_values'],
                'j_grid': _bw_file['j_grid'],
                'heatmap_kV': _bw_file['heatmap_kV'],
                'j_max_values': _bw_file['j_max_values'],
                'Delta_meV': float(_bw_file['Delta_meV']),
                'tau': float(_bw_file['tau']),
                'a_M': float(_bw_file['a_M']),
                'F_c': float(_bw_file['F_c']),
            }
        else:
            print(f"Warning: {_filename}.npz not found")

    # Load bandwidth sweep data (β = 0.5, Δ = 10-80 meV)
    bandwidth_sweep = None
    _sweep_filename = 'heatmap_bandwidth_sweep_beta05'
    _sweep_file = None

    if os.path.exists(f'{_sweep_filename}.npz'):
        _sweep_file = np.load(f'{_sweep_filename}.npz')
    else:
        _sweep_file = load_embedded(_sweep_filename)

    if _sweep_file is not None:
        bandwidth_sweep = {
            'Delta_values': _sweep_file['Delta_values'],
            'j_grid': _sweep_file['j_grid'],
            'heatmap_kV': _sweep_file['heatmap_kV'],
            'j_max_values': _sweep_file['j_max_values'],
            'beta': float(_sweep_file['beta']),
            'tau': float(_sweep_file['tau']),
            'a_M': float(_sweep_file['a_M']),
            'F_c': float(_sweep_file['F_c']),
        }
    else:
        print("Warning: heatmap_bandwidth_sweep_beta05.npz not found")
    return bandwidth_data, bandwidth_sweep


@app.cell
def tab8_bandwidth_comparison(
    bandwidth_data,
    bandwidth_sweep,
    go,
    make_subplots,
    ...
    """Tab comparing heatmaps for different bandwidths."""

    # Handle empty data case at the end
    _has_data = bool(bandwidth_data)
    _has_sweep = bandwidth_sweep is not None

    # --- Figure 0: Bandwidth Sweep Heatmap (β = 0.5, Δ = 10-80 meV) ---
    if _has_sweep:
        fig_sweep = go.Figure()

        fig_sweep.add_trace(go.Heatmap(
            x=bandwidth_sweep['j_grid'],
            y=bandwidth_sweep['Delta_values'],
            z=bandwidth_sweep['heatmap_kV'],
            colorscale='Viridis',
            zmin=0, zmax=25,
            colorbar=dict(title='r_diff [kV·m/A]')
        ))

        # Add j_max curve as overlay
        fig_sweep.add_trace(go.Scatter(
            x=bandwidth_sweep['j_max_values'],
            y=bandwidth_sweep['Delta_values'],
            mode='lines+markers',
            line=dict(color='red', width=3),
        ...
        fig_sweep.update_layout(
            title=f'Differential Resistance r_diff(j, Δ) at β = {bandwidth_sweep["beta"]:.1f}',
            xaxis_title='j [A/m]',
            yaxis_title='Δ [meV]',
            height=500,
            template='plotly_white'
        ...
        # Add scaling line trace
        fig_jmax_scaling = go.Figure()

        fig_jmax_scaling.add_trace(go.Scatter(
            x=bandwidth_sweep['Delta_values'],
            y=bandwidth_sweep['j_max_values'],
            mode='lines+markers',
        ...
        # Linear fit
        _Delta_arr = bandwidth_sweep['Delta_values']
        _jmax_arr = bandwidth_sweep['j_max_values']
        _slope = (_jmax_arr[-1] - _jmax_arr[0]) / (_Delta_arr[-1] - _Delta_arr[0])
        _intercept = _jmax_arr[0] - _slope * _Delta_arr[0]
        _fit = _slope * _Delta_arr + _intercept
        ...
        fig_jmax_scaling.add_trace(go.Scatter(
            ...
            name=f'Linear fit: {_slope:.2f} A/m per meV'
        ))

        fig_jmax_scaling.update_layout(
            title='j_max Scaling with Bandwidth at β = 0.5',
            xaxis_title='Δ [meV]',
            yaxis_title='j_max [A/m]',
            height=400,
            template='plotly_white',
            legend=dict(x=0.02, y=0.98)
        )

        # 3D surface for sweep
        fig_sweep_3d = go.Figure()

        fig_sweep_3d.add_trace(go.Surface(
            x=bandwidth_sweep['j_grid'],
            y=bandwidth_sweep['Delta_values'],
            z=bandwidth_sweep['heatmap_kV'],
            colorscale='Viridis',
            colorbar=dict(title='r_diff [kV·m/A]', len=0.7)
        ))

        fig_sweep_3d.update_layout(
            title='3D View: r_diff(j, Δ) at β = 0.5',
            scene=dict(
                xaxis_title='j [A/m]',
                yaxis_title='Δ [meV]',
                zaxis_title='r_diff [kV·m/A]',
    ...
    fig_heatmaps = make_subplots(
        rows=1, cols=3,
        subplot_titles=[f'Δ = {d} meV' for d in [50, 60, 70]],
        horizontal_spacing=0.08
    )

    for _col, _Delta in enumerate([50, 60, 70], 1):
        if _Delta in bandwidth_data:
            _data = bandwidth_data[_Delta]
            fig_heatmaps.add_trace(
                go.Heatmap(
                    ...
                    zmin=0, zmax=25,
                    showscale=(_col == 3),
                    colorbar=dict(title='r_diff [kV·m/A]', len=0.8) if _col == 3 else None
                ),
                row=1, col=_col
            )

    fig_heatmaps.update_layout(
        title='Differential Resistance r_diff(j, β) for Different Bandwidths',
        height=450,
        template='plotly_white'
    )
    for _col in range(1, 4):
        fig_heatmaps.update_xaxes(title_text='j [A/m]', row=1, col=_col)
        fig_heatmaps.update_yaxes(title_text='β' if _col == 1 else '', row=1, col=_col)

    # --- Figure 2: j_max vs beta for all bandwidths ---
    fig_jmax = go.Figure()

    _colors = {'50': 'blue', '60': 'green', '70': 'red'}
    for _Delta in [50, 60, 70]:
        if _Delta in bandwidth_data:
            _data = bandwidth_data[_Delta]
            fig_jmax.add_trace(go.Scatter(
                x=_data['beta_values'],
                y=_data['j_max_values'],
                mode='lines+markers',
                name=f'Δ = {_Delta} meV',
                line=dict(color=_colors[str(_Delta)], width=2),
                marker=dict(size=6)
            ))

    fig_jmax.update_layout(
        title='Maximum Current j_max vs Filling for Different Bandwidths',
        xaxis_title='β (filling fraction)',
        yaxis_title='j_max [A/m]',
        height=400,
        template='plotly_white',
    ...
    # --- Figure 3: Peak j_max and optimal beta ---
    _peak_data = []
    for _Delta in [50, 60, 70]:
        if _Delta in bandwidth_data:
            _data = bandwidth_data[_Delta]
            _idx_peak = np.argmax(_data['j_max_values'])
            _peak_data.append({
                'Delta': _Delta,
                'j_max_peak': _data['j_max_values'][_idx_peak],
                'beta_optimal': _data['beta_values'][_idx_peak]
            })

    fig_scaling = make_subplots(
        rows=1, cols=2,
        subplot_titles=['Peak j_max vs Bandwidth', 'Optimal Filling vs Bandwidth']
    )

    _Deltas = [d['Delta'] for d in _peak_data]
    _jmax_peaks = [d['j_max_peak'] for d in _peak_data]
    _beta_opts = [d['beta_optimal'] for d in _peak_data]

    fig_scaling.add_trace(
        go.Scatter(
            x=_Deltas, y=_jmax_peaks,
            mode='lines+markers',
    ...
    _slope = (_jmax_peaks[-1] - _jmax_peaks[0]) / (_Deltas[-1] - _Deltas[0])
    _intercept = _jmax_peaks[0] - _slope * _Deltas[0]
    _fit_line = [_slope * d + _intercept for d in _Deltas]

    fig_scaling.add_trace(
        go.Scatter(
            x=_Deltas, y=_fit_line,
            mode='lines',
    ...
    fig_scaling.update_xaxes(title_text='Δ [meV]', row=1, col=1)
    fig_scaling.update_xaxes(title_text='Δ [meV]', row=1, col=2)
    fig_scaling.update_yaxes(title_text='j_max,peak [A/m]', row=1, col=1)
    fig_scaling.update_yaxes(title_text='β_optimal', row=1, col=2)
    ...
    # --- Figure 4: r_diff line traces comparison ---
    fig_rdiff_traces = go.Figure()

    _beta_show = 0.70  # Show for this filling
    for _Delta in [50, 60, 70]:
        if _Delta in bandwidth_data:
            _data = bandwidth_data[_Delta]
            # Find closest beta
            _idx = np.argmin(np.abs(_data['beta_values'] - _beta_show))
            _rdiff = _data['heatmap_kV'][_idx, :]
            _valid = np.isfinite(_rdiff)
            fig_rdiff_traces.add_trace(go.Scatter(
                x=_data['j_grid'][_valid],
                y=_rdiff[_valid],
                mode='lines',
                name=f'Δ = {_Delta} meV',
                line=dict(color=_colors[str(_Delta)], width=2)
    ...
    fig_rdiff_traces.update_layout(
        title=f'r_diff(j) at β = {_beta_show:.2f} for Different Bandwidths',
        xaxis_title='j [A/m]',
        yaxis_title='r_diff [kV·m/A]',
        height=400,
        template='plotly_white',
    ...
    # --- Figure 5: 3D surface comparison ---
    fig_3d = go.Figure()

    for _i, _Delta in enumerate([50, 60, 70]):
        if _Delta in bandwidth_data:
            _data = bandwidth_data[_Delta]
            # Offset in z for visibility
            _z_offset = _i * 30
            fig_3d.add_trace(go.Surface(
                x=_data['j_grid'],
                y=_data['beta_values'],
                z=_data['heatmap_kV'] + _z_offset,
                colorscale='Viridis',
                showscale=False,
                opacity=0.8,
                name=f'Δ = {_Delta} meV'
    ...
    fig_3d.update_layout(
        title='3D View: r_diff Surfaces for Different Bandwidths',
        scene=dict(
            xaxis_title='j [A/m]',
            yaxis_title='β',
            zaxis_title='r_diff [kV·m/A] (offset)',
            camera=dict(eye=dict(x=1.5, y=-1.5, z=0.8))
        ),
        height=500,
        template='plotly_white'
    ...
    tab8_content = mo.vstack([
        mo.md(r"""
    # Bandwidth Sweep: r_diff vs Current vs Bandwidth

    ## Part 1: Fixed Filling (β = 0.5), Variable Bandwidth (10-80 meV)

    This section shows how the differential resistance evolves with bandwidth at half-filling:
        """),

        fig_sweep if _has_sweep else mo.md("*Sweep data not available*"),

        mo.md(r"""
    The **white dashed line** traces $j_{\max}(\Delta)$, the maximum current before instability.

    **Key observation:** $j_{\max}$ scales linearly with bandwidth: $j_{\max} \propto \Delta$
        """),

        fig_jmax_scaling if _has_sweep else mo.md(""),
    ...
    ### 3D Surface View
        ...
        fig_sweep_3d if _has_sweep else mo.md(""),

        mo.md(r"""
    ---

    ## Part 2: Variable Filling, Fixed Bandwidths (Δ = 50, 60, 70 meV)

    This section compares the differential resistance heatmaps and current characteristics
    for three different bandwidth values while keeping other parameters fixed:

    | Parameter | Value |
    |-----------|-------|
    | τ (scattering time) | 1 ps |
    | a_M (moiré period) | 14.5 nm |
    | F_c (critical field) | 45.4 kV/m |

    ---

    ## Side-by-Side Heatmaps

    Larger bandwidth → higher maximum current and broader stable region:
        """),
    ...
    The $j_{\max}(\beta)$ curves show how the peak current scales with bandwidth:
        """),
    ...
    **Key observations:**
    ...
    1. **Peak current scales linearly with bandwidth:** $j_{\max} \propto \Delta$
       - This is expected since $j \propto v_{\max} \propto \Delta$

    2. **Optimal filling is nearly constant:** $\beta_{\text{opt}} \approx 0.70$
       - The velocity distribution geometry is bandwidth-independent

    3. **The shape of r_diff(j) curves is similar** — they're rescaled versions
        """),
    ...
    ## r_diff Line Traces at β = 0.70

    Comparing the differential resistance curves at the optimal filling:
        """),
    ...
    The surfaces are vertically offset for clarity:
        """),
    ...
    ### Why does j_max scale with Δ?

    The maximum current is:
    $$j_{\max} \propto n \cdot e \cdot v_{\max} \cdot f(\beta)$$

    where $v_{\max} = \frac{\Delta \cdot a_M}{2\hbar}$ is the maximum group velocity.

    Since $v_{\max} \propto \Delta$, we expect $j_{\max} \propto \Delta$.

    ### Why is β_optimal constant?

    The optimal filling is determined by the **geometry of the velocity distribution**
    in k-space, which depends only on the dispersion shape (C3 symmetry), not its amplitude.

    ### Implications for experiments

    - **Larger bandwidth** (higher twist angle): Higher currents, easier to measure
    - **Smaller bandwidth** (magic angle): Lower currents, but potentially more interesting physics
    - **Critical field F_c** is bandwidth-independent (depends only on τ and a_M)
        """)
    ])

    # Handle missing data case
    if not _has_data:
        tab8_content = mo.md("**Error:** Bandwidth heatmap data not found. Run `generate_bandwidth_heatmaps.py` first.")
    return (tab8_content,)
